...
To train the GPT-2 image captioning model with multiple different configurations, run the following command:
```bash
uv run -m sweep
```
This will commence batch training and perform evalutaion on each model based on the configurations set in [`automation_config.yml`](./automation_config.yml). This will create new checkpoint folders per model under the parent directory: `checkpoints/`. The configuration file for the model, train and validation results will all be saved in their respective checkpoint folders.

To add new, remove or update existing configurations, simply edit the config file: [`automation_config.yml`](./automation_config.yml).

NOTE 1: the structure of the hyperparameters has to follow the same structure in the base model configuration file: [`config.yml`](./config.yml).

NOTE 2: description is for own reference (Optional)
...
```
Run the visualization script with the correct paths for the predictions, images directory and annotations.
For example:
```bash
...
For help run:
```bash
uv run visualize_my_results.py --help
```

This will:
- Load your predictions from the specified file
- Compare with COCO val2017 ground truth captions
- Launch an interactive viewer at http://localhost:5151
