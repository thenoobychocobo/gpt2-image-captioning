...
if __name__ == "__main__":
    ...
    for _, i in enumerate(checkpoint_dirs):
        ...
        best_model_files = [f for f in all_files if f.startswith("best_model_") and f.endswith(".pt")]
        ...
        if cfg.retrieval_augmentation:
            ...
            predictions, metrics = testing_pipeline(cfg, model, tokenizer, checkpoint_dir)
        
        # Save predictions and metrics
        with open(os.path.join(checkpoint_dir, f"test_predictions_{cfg_idx}.txt"), "w") as f:
            json.dump(predictions, f)
        with open(os.path.join(checkpoint_dir, f"test_metrics_{cfg_idx}.txt"), "w") as f:
            f.write(str(metrics))
