...
def load_dinov3_models(
    ...
    for file_name in os.listdir(model_weights_dir):
        if file_name == WEIGHTS_FILE:
            vision_head_and_text_encoder_path = os.path.join(model_weights_dir, file_name)
    ...
    if vision_head_and_text_encoder_path is None:
        raise FileNotFoundError(f"Could not find '{WEIGHTS_FILE}' in directory '{model_weights_dir}'")
    if pretrain_lvd1689m_path is None:
        raise FileNotFoundError(f"Could not find '{BACKBONE_WEIGHTS_FILE}' in directory '{model_weights_dir}'")
...
# why do i copy the preprocessor from the repo? because the torch.hub.load does not help on getting it, it only helps on getting the model and tokenizer.
...
if __name__ == "__main__":
    ...
    dino_model, _ = load_dinov3_models(model_weights_dir="path/to/dinov3/weights", device=device)
